...
public class RemoveParenthesisCleanUp extends AbstractCleanUpRule {
	...
	public boolean visit(final ParenthesizedExpression visited) {
		...
		if (expressionWithoutParentheses != null) {
			...
			rewrite.replace(visited, ASTNodes.createMoveTarget(rewrite, expressionWithoutParentheses), group);
			return false;
		}
	...
	private boolean isParenthesesUselessForParent(final ASTNode parent, final ParenthesizedExpression node) {
		...
		case ASTNode.METHOD_INVOCATION:
			...
			return methodInvocation.arguments().contains(node) || canRemoveParenthesesAroundExpression(methodInvocation, node);

		case ASTNode.SUPER_METHOD_INVOCATION:
			SuperMethodInvocation superMethodInvocation= (SuperMethodInvocation) parent;
			return superMethodInvocation.arguments().contains(node);
