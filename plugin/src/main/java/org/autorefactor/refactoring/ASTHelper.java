...
public final class ASTHelper {
     ...
     *
     * @param node the start node
     * @param ancestorClazzes the required ancestor's types
     * @return the first ancestor of the provided node which has any of the required types
     */
    public static ASTNode getAncestor(ASTNode node, Class<? extends ASTNode>... ancestorClazzes) {
        if (node == null || node.getParent() == null) {
            String clazzes = Arrays.toString(ancestorClazzes);
            throw new IllegalStateException(node,
                    "Could not find any ancestor for " + clazzes + " and node " + node);
        }
        final ASTNode parent = node.getParent();
        for (Class<?> ancestorClazz : ancestorClazzes) {
            if (ancestorClazz.isAssignableFrom(parent.getClass())) {
                return parent;
            }
        }
        return getAncestor(parent, ancestorClazzes);
    }
